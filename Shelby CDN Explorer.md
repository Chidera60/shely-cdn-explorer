...
2. **SDK Interaction:** The Next.js client uses ShelbyClient to upload the Uint8Array to the Shelby MAINNET (or MAINNET configurable via .env). An ephemeral Aptos account signs the transaction.
...
- NEXT_PUBLIC_SHELBY_API_KEY: Required for SDK initialization (if using authenticated gateways).
